...
void StreamEngine::startStream(AAudioStream *stream) {
    ...
    if (result != AAUDIO_OK) {
        LOGE("  Error AAudioStream_requestStart : %s", AAudio_convertResultToText(result));
    }

    result = AAudioStream_waitForStateChange(stream, inputState, &nextState, timeoutNanos);

    if (result != AAUDIO_OK) {
        LOGE("  Error AAudioStream_waitForStateChange : %s", AAudio_convertResultToText(result));
    }
}
...
//            }
    ...
    LOGI("  writeBufferToStreamer result : %s", AAudio_convertResultToText(result));
